...
import {
  VoucherModule_Body_BookOne,
  VoucherModule_Body_CreateOne,
  VoucherModule_Body_UpdateOne,
} from "./types/body.types";
import {
  VoucherModule_Response_BookOne,
  VoucherModule_Response_CreateOne,
  VoucherModule_Response_EnshrineOne,
  ...
  VoucherModule_Response_GetGuidanceForRevenueAccounts,
  VoucherModule_Response_GetMany,
  VoucherModule_Response_GetManyAccountGuides,
  VoucherModule_Response_GetOne,
  VoucherModule_Response_ResetOnesStatusToDraft,
  VoucherModule_Response_ResetOnesStatusToOpen,
  VoucherModule_Response_UpdateOne,
  VoucherModule_Response_Upload,
...
The only valid status values for this endpoint are 50 (draft) and 100 (open). You can only update draft vouchers. If you have to, you can downgrade the status by calling resetToOpen (from paid) and resetToDraft (from open).
   ...
   */
  async createOne(body: VoucherModule_Body_CreateOne) {
    return await new API(this.apiKey).request<VoucherModule_Response_CreateOne>(
...
If you provided all necessary parameters and kept all of them in the right order, the file will be attached to your voucher.
  ...
  async upload(file: FormData) {
    return await new API(this.apiKey).request<VoucherModule_Response_Upload>(
  ...
  async getMany(
    ...
    return await new API(this.apiKey).request<VoucherModule_Response_GetMany>(
      "/Voucher",
      queryObj,
      {
        method: "GET",
  ...
  async getOne(voucherId: number) {
    return await new API(this.apiKey).request<VoucherModule_Response_GetOne>(
      `/Voucher/${voucherId}`,
      undefined,
      { method: "GET" }
    );
...
You can not change the status using this endpoint.
   ...
   */
  async updateOne(voucherId: number, body: VoucherModule_Body_UpdateOne) {
    return await new API(this.apiKey).request<VoucherModule_Response_UpdateOne>(
      `/Voucher/${voucherId}`,
      undefined,
      {
        method: "PUT",
...
Enshrined vouchers cannot be changed. This operation cannot be undone.
  ...
  async enshrineOne(voucherId: number) {
    return await new API(
      this.apiKey
    ).request<VoucherModule_Response_EnshrineOne>(
...
For this reason, you can have a look at it here and all you need to do is to change "Invoice" into "Voucher" in the URL.
   ...
   */
  async bookOne(voucherId: number, body: VoucherModule_Body_BookOne) {
    return await new API(this.apiKey).request<VoucherModule_Response_BookOne>(
...
To change to higher status use Voucher/{voucherId}/bookAmount. To change to lower status use Voucher/{voucherId}/resetToDraft.
  ...
  async resetOnesStatusToOpen(voucherId: number) {
    return await new API(
      this.apiKey
    ).request<VoucherModule_Response_ResetOnesStatusToOpen>(
...
To change to higher status use /Voucher/Factory/saveVoucher.
  ...
  async resetOnesStatusToDraft(voucherId: number) {
    return await new API(
      this.apiKey
    ).request<VoucherModule_Response_ResetOnesStatusToDraft>(
  ...
  async getManyAccountGuides() {
    return await new API(
      this.apiKey
    ).request<VoucherModule_Response_GetManyAccountGuides>(
  ...
  async getGuidanceByAmountNumber(accountNumber: number) {
    return await new API(
      this.apiKey
    ).request<VoucherModule_Response_GetGuidanceByAmountNumber>(
  ...
  async getGuidanceByTaxRule(taxRule: string) {
    return await new API(
      this.apiKey
    ).request<VoucherModule_Response_GetGuidanceByTaxRule>(
  ...
  async getGuidanceForRevenueAccounts() {
    return (
      await new API(this.apiKey).request
    )<VoucherModule_Response_GetGuidanceForRevenueAccounts>(
  ...
  async getGuidanceForExpenseAccounts() {
    return await new API(
      this.apiKey
    ).request<VoucherModule_Response_GetGuidanceForExpenseAccounts>(
